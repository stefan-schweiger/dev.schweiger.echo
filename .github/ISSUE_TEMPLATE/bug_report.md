...
- type: markdown
  attributes:
  value: | ## Before you open an issue
      ...
      1. **Enable 2FA with an authenticator app**
          Amazon requires 2-step verification using an authenticator app (e.g. Google Authenticator, Microsoft Authenticator). **SMS or email-based 2FA will NOT work.**
...
- type: checkboxes
  ...
  options: - label: I have 2FA enabled using an **authenticator app** (not SMS/email)
  required: true - label: I have tried Disconnect → Reset → Restart in the app settings
  required: true - label: I have verified the correct Amazon website/region is selected
  required: true - label: I do not have another Alexa app installed/enabled on Homey
  required: true - label: I am running the latest version of the Echo app
